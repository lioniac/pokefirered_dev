...
// Const Data
static const struct FollowerSpriteGraphics sFollowerAlternateSprites[] =
{
    //FORMAT:
    ...
    [0] = 
    {
        ...
        .bikeId = OBJ_EVENT_GFX_GREEN_BIKE,
...
static u16 GetFollowerSprite(void)
{
    ...
    case FOLLOWER_SPRITE_INDEX_BIKE:
        for (i = 0; i < NELEMS(sFollowerAlternateSprites); i++)
        {
            if (sFollowerAlternateSprites[i].normalId == gSaveBlock1Ptr->follower.graphicsId)
                return sFollowerAlternateSprites[i].bikeId;
        }
        break;
    case FOLLOWER_SPRITE_INDEX_SURF:
        ...
        {
            if (sFollowerAlternateSprites[i].normalId == gSaveBlock1Ptr->follower.graphicsId)
                return sFollowerAlternateSprites[i].surfId;
        }
        break;
    case FOLLOWER_SPRITE_INDEX_UNDERWATER:
        ...
        {
            if (sFollowerAlternateSprites[i].normalId == gSaveBlock1Ptr->follower.graphicsId)
                return sFollowerAlternateSprites[i].underwaterId;
        }
        break;
    }
...
static void TryUpdateFollowerSpriteUnderwater(void)
{
    if (gMapHeader.mapType == MAP_TYPE_UNDERWATER)
    {
        ...
        follower->fieldEffectSpriteId = CreateDiveBobbingSprite(follower->spriteId);
    }
}
...
static u8 DetermineFollowerState(struct ObjectEvent* follower, u8 state, u8 direction)
{
    ...
    // slow slide, or waterfall
    case MOVEMENT_ACTION_FACE_DOWN_SLOW ... MOVEMENT_ACTION_FACE_RIGHT_SLOW:
        if (PlayerIsUnderAndFacingWaterfall())
            return MOVEMENT_INVALID;

        RETURN_STATE(MOVEMENT_ACTION_FACE_DOWN_SLOW, direction);
    
    // slide down (eg on ice)
    case MOVEMENT_ACTION_WALK_FASTEST_DOWN ... MOVEMENT_ACTION_WALK_FASTEST_RIGHT:
        // mach bike. unsure if used in FR
        if (MetatileBehavior_IsIce(follower->currentMetatileBehavior) || MetatileBehavior_IsTrickHouseSlipperyFloor(follower->currentMetatileBehavior))
            follower->disableAnim = TRUE;
    ...
    // acro bike -> these technically exist in FR but are unused.
    /*
...
void FollowMe_HandleBike(void)
{
	...
	if (gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_BIKE && FollowerCanBike() && gSaveBlock1Ptr->follower.comeOutDoorStairs != 1) //Coming out door
...
{
	if ((gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_BIKE) && FollowerCanBike())
    ...
    for (eventObjId = 0; eventObjId < OBJECT_EVENTS_COUNT; eventObjId++) //For each NPC on the map
    {
        ...
        if (gObjectEvents[eventObjId].localId == localId)
        {
            ...
            if (CheckFollowerFlag(FOLLOWER_FLAG_CUSTOM_FOLLOW_SCRIPT))
                script = (const u8 *)GetScriptDataPointer(0);
            else
